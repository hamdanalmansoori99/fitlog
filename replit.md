...
### Data Management
- **Workouts**: Comprehensive logging for 8 activity types, gym exercise tracker with autocomplete (powered by 209-exercise library across 12 categories: chest, back, shoulders, biceps, triceps, legs, glutes, abs, forearms, cardio, mobility, bodyweight), workout templates, and history. Exercise detail screen shows form instructions, common mistakes with warning icons, animated 3D placeholder skeleton, muscles worked, equipment, and a log CTA.
- **Meals**: Multi-item food logging with macros, calorie summaries, nutrition statistics, barcode scanner (via Open Food Facts API), AI photo analysis with non-food detection, AI single-day meal plan generator (Premium), 7-day AI weekly meal planner (Premium) via `/meals/weekly-plan` — stored in Zustand+AsyncStorage, allows per-meal logging, and AI grocery list generation from weekly plan (Premium) with interactive checklist modal.
- **Progress**: Tracks streaks, activity breakdown, weight charts, PRs, body measurements (with per-metric trend sparklines for BF%, waist, chest, hips, arms in a 2-column grid with delta badges), workout history calendar, and progress photos. Workout detail includes a share card modal for screenshot sharing.
...
- **Home Screen 3-Zone Layout**: The home screen is organized into three zones: (1) Hero — greeting, enlarged 150px CalorieRing with macro bars (Protein/Carbs/Fat), and Today's Workout card; (2) Quick Actions — 3 action buttons (Scan Meal, Start Workout, Ask Coach) + 4 horizontal coach prompt chips that navigate to `/coach/chat` with a pre-filled `prompt` query param; (3) Secondary Feed — streaks card, don't-break-streak banner, AI insight card (top insight from `computeGoalInsights`), and weekly report placeholder. Coach chat (`/coach/chat`) accepts a `prompt` query param — auto-sends when conversation is empty, pre-fills input when conversation exists.
...
Full bilingual support (English + Arabic) with RTL layout. Uses `i18next` + `react-i18next` + `expo-localization`. Translation files at `artifacts/fitlog/i18n/locales/en.ts` (~1300+ lines) and `ar.ts` (fully synced Arabic). All 28 screen files, 12 components, and 3 utility lib files use translation calls. React components use `useTranslation()` with `t()` calls; non-React utility files (`lib/notifications.ts`, `lib/nutritionCoach.ts`, `lib/goalInsights.ts`) import `i18n` directly from `@/i18n` and use `i18n.t()`. Language switching via `useLanguage` hook persists to API, updates RTL via `I18nManager`, reloads on web via `window.location.reload()`, and reloads on native via `expo-updates` `Updates.reloadAsync()`. Server-driven language hydration reads `GET /settings` on login to restore saved preference. AI coach responds in Arabic when `language === "ar"`.
